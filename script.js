/* ============================================================
   script.js — Sécurité Informatique Interactive Course
...
function flashOverlay() {
  ...
  o.style.cssText = 'position:fixed;inset:0;background:var(--accent);opacity:0.05;pointer-events:none;z-index:9999;transition:opacity 0.4s ease;';
        ...
        setTimeout(() => {
          ...
          if (newC) {
            ...
            requestAnimationFrame(() => {
              ...
              newC.style.transform = 'translateY(0)';
            });
          }
        }, 180);
      }
...
function initSidebar() {
  ...
  backdrop.style.cssText = 'display:none;position:fixed;inset:0;background:rgba(40,45,20,0.5);z-index:850;opacity:0;transition:opacity 0.3s ease;backdrop-filter:blur(2px);';
  ...
  sidebar.querySelectorAll('.nav-link').forEach(l => {
    l.addEventListener('click', () => { if (window.innerWidth <= 1024) close(); });
  });
}
...
function initSearch() {
  ...
  input.addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      const nx = focused ? focused.nextElementSibling : items[0];
      setKFocus(focused, nx, items);
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      const pv = focused ? focused.previousElementSibling : items[items.length - 1];
      setKFocus(focused, pv, items);
    } else if (e.key === 'Enter' && focused) focused.click();
  });
  function setKFocus(o, n, all) {
    all.forEach(i => i.classList.remove('keyboard-focus'));
    if (n) { n.classList.add('keyboard-focus'); n.scrollIntoView({ block: 'nearest' }); }
  }
  function renderResults(matches, q) {
    results.innerHTML = '';
    if (!matches.length) {
      results.innerHTML = `<div class="search-result-item" style="color:var(--text-muted)">Aucun résultat pour "${q}"</div>`;
    ...
    matches.forEach(item => {
      ...
      const match = item.text.slice(idx, idx + q.length);
      const after = item.text.slice(idx + q.length, idx + q.length + 60);
      div.innerHTML = `<div style="font-weight:700;color:var(--accent);font-size:12px;margin-bottom:2px">${item.tab}</div><div>${esc(before)}<strong>${esc(match)}</strong>${esc(after)}${after.length >= 60 ? '…' : ''}</div>`;
      div.addEventListener('click', () => {
        ...
        setTimeout(() => {
          ...
          if (t) {
            ...
            t.style.transition = 'background 0.4s ease';
...
function buildSearchIndex() {
  ...
  const tabs = [
    ...
    { id: 'quiz', label: '🎯 Quiz', contentId: 'content-quiz' }
  ];
  tabs.forEach(tab => {
    ...
    container.querySelectorAll('h2,h3,h4,h5,section').forEach(el => {
      if (el.tagName === 'SECTION') {
        const t = el.querySelector('h3,h4,h5')?.textContent?.trim();
        if (t) index.push({ id: el.id, text: t, tab: tab.label, tabId: tab.id });
      } else if (el.textContent.trim()) {
        index.push({ id: el.id || el.closest('section')?.id || '', text: el.textContent.trim(), tab: tab.label, tabId: tab.id });
      }
    });
  ...
  return index.filter(item => {
    const k = item.id + item.text.slice(0, 30);
  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        navLinks.forEach(l => {
          const isA = l.getAttribute('href') === `#${id}`;
          l.classList.toggle('active', isA);
          if (isA) l.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        });
      }
    });
...
function initScrollAnimations() {
  const els = document.querySelectorAll('.card,.mini-card,.cia-item,.definition-box,.important-box,.warning-box,.remember-box,.example-box,.formula-box,.sand-box');
  els.forEach(el => {
    el.style.opacity = '0';
...
function initProgressBar() {
  ...
  bar.style.cssText = 'position:fixed;top:0;left:0;height:3px;width:0%;background:linear-gradient(90deg,var(--accent),var(--olive));z-index:1100;transition:width 0.1s linear;box-shadow:0 0 6px var(--accent);';
...
function initTableRowHighlight() {
  document.querySelectorAll('.styled-table tbody tr').forEach(row => {
    ...
    row.addEventListener('click', () => {
      ...
      row.style.background = isH ? '' : 'var(--accent-light)';
      row.style.borderLeft = isH ? '' : '3px solid var(--accent)';
      row.dataset.highlighted = isH ? 'false' : 'true';
    });
  });
}
...
function initFormulaPulse() {
  const obs = new IntersectionObserver(entries => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        const f = e.target.querySelector('.formula');
        if (f) {
          f.style.transition = 'transform 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
          f.style.transform = 'scale(1.06)';
          setTimeout(() => { f.style.transform = 'scale(1)'; }, 450);
        }
        obs.unobserve(e.target);
      }
    });
...
function getChapterClass(chapter) {
  ...
  if (chapter.includes('5')) return 'badge-ch5';
  return '';
}

function selectAnswer(qIdx, optIdx) {
  if (quizState.answers[qIdx] !== null) return;
...
function finishQuiz() {
  ...
  let icon, title, msg;
  if (pct >= 80) {
    icon = '🏆'; title = 'Excellent travail !';
    msg = `Vous maîtrisez très bien le cours de cybersécurité. Score : ${score}/${total} (${pct}%)`;
  } else if (pct >= 60) {
    icon = '👍'; title = 'Bon résultat !';
    msg = `Bonne maîtrise du cours. Quelques révisions s'imposent. Score : ${score}/${total} (${pct}%)`;
  } else if (pct >= 40) {
    icon = '📚'; title = 'Résultat moyen';
    msg = `Des efforts de révision sont nécessaires. Score : ${score}/${total} (${pct}%)`;
  } else {
    icon = '💪'; title = 'À retravailler';
    msg = `Relisez attentivement le cours et réessayez ! Score : ${score}/${total} (${pct}%)`;
  }
  ...
  bd.innerHTML = '<h4 style="margin-bottom:12px;color:var(--text-primary)">Résultats par chapitre :</h4>';
  Object.entries(chapters).forEach(([ch, data]) => {
    const p = Math.round((data.correct / data.total) * 100);
    const barColor = p >= 80 ? 'var(--green)' : p >= 60 ? 'var(--accent)' : p >= 40 ? 'var(--yellow)' : 'var(--red)';
    bd.innerHTML += `
      <div class="breakdown-item">
        <div class="breakdown-label">${ch}</div>
        <div class="breakdown-bar-wrap">
          <div class="breakdown-bar" style="width:${p}%;background:${barColor}"></div>
        </div>
